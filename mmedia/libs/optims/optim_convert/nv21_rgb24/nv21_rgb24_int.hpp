...
#  define NV212RGB24(px_per_cycle)                                                                             \
    ...
    get_params (info, &yuy2, &rgb24);                                                                          \
                                                                                                               \
    const std::uint32_t off_beg_uv = yuy2->width_ * yuy2->height_ * (info.count_threads_ - info.indx_thread_); \
    const std::uint32_t off_uv     = yuy2->width_ * (yuy2->height_ >> 1) * info.indx_thread_;                  \
