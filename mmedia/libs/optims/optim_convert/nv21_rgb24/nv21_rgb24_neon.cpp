...
neon (::libs::optim::io::MCallInfo& info)
{
  ...
  for (uint32_t indx_y = 0; indx_y < yuy2->height_; ++indx_y)
  {
    const std::uint8_t* prev_uv_buf = uv_buf;

    for (uint32_t indx_x = 0; indx_x < yuy2->width_; indx_x += ppc)
    {
      y8   = vld1_u8 (y_buf);
      uv8  = vld1_u8 (uv_buf);
      vu8  = vrev16_u8 (uv8);
      y16  = vreinterpretq_s16_u16 (vmovl_u8 (y8));
      vu16 = vreinterpretq_s16_u16 (vmovl_u8 (vu8));
      uv16 = vreinterpretq_s16_u16 (vmovl_u8 (uv8));
      ...
      // vrgb24.val[0]  = vdup_n_u8(255 );//debug
      ...
      // vrgb24.val[2]  = vdup_n_u8(0);//debug
