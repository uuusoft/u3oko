...
struct MCallInfo final {
  ...
  std::uint32_t indx_thread_;       //< Индекс потока, которому принадлежит данная копия структуры
  std::uint32_t count_threads_;     //< Количество рабочих потоков, используемых при данном вызове
  std::uint32_t full_height_src_;   //< Полная высота буфера источника, не может быть получена простым перемножением количества потоков на высоту части, т.к. высоты разные
  std::uint32_t full_height_dst_;   //< Полная высота буфера назначения, не может быть получена простым перемножением количества потоков на высоту части, т.к. высоты разные
