...
struct MCallInfo final {
  ...
  MCallInfo (const std::string& func_id = std::string (std::source_location::current ().function_name ())) :
    func_id_ (func_id)
  {
    srcs_.reserve (4);
    dsts_.reserve (4);
  ...
  std::string   func_id_         = "unknown";   //<
  std::uint32_t thread_indx_     = 0;           //< Индекс потока, которому принадлежит данная копия структуры
