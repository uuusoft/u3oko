...
namespace libs::optim::s16bit::conv::base::c11x11::consts
{
constexpr std::int32_t pxs_per_cycle = 1;   //<
}

namespace libs::optim::s16bit::conv::base::c11x11
{
#  define PROCESS_STR(Str)                                                                                                     \
    str_mask = _mm256_lddqu_si256 (::libs::helpers::casts::reinterpret_cast_helper< const __m256i* > (&(*mask).get (0, Str))); \
    data     = _mm256_lddqu_si256 (::libs::helpers::casts::reinterpret_cast_helper< const __m256i* > (csstr));                 \
...
{
  _mm256_zeroupper ();
  move_alg< TModAvx2CalcObj, cores::values_core_type, TPostProcessor, consts::size_core > (info);
  _mm256_zeroupper ();
}
...
{
  _mm256_zeroupper ();
  move_alg< TAvx2CalcObj, cores::values_core_type, TPostProcessor, consts::size_core > (info);
  _mm256_zeroupper ();
}
}   // namespace libs::optim::s16bit::conv::base::c11x11
