...
constexpr std::int32_t max_count_msgs = 10 * 1024;         //< Максимальное количество сообщений в очередях передачи
constexpr std::int32_t max_size_msg   = 1 * 1024 * 1024;   //< Максимальный размер сообщения в байтах

#if defined(U3_OS_WIN32_DESKTOP) || defined(U3_OS_GNU_LINUX) || defined(U3_OS_MACX_DESKTOP)
constexpr std::int32_t size_shared_mem_bytes           = 512 * 1024 * 1024;
...
constexpr std::int32_t size_shared_mem_bytes           = 64 * 1024 * 1024;
constexpr std::int32_t ms_wait_delay_recv_msg          = 1;
constexpr std::int32_t ms_wait_delay_special_msg_cycle = 0;
