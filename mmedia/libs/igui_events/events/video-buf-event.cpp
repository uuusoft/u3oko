...
{
VideoBufEvent::VideoBufEvent (const Acessor& ph, const Bufs::ptr& buf, int id) :
  buf_ (buf),
  id_ (id)
{
...
{
  U3_CHECK_COPY_EVENT (VideoBufEvent);
  super::copy_int (src);
  buf_ = dsrc->buf_;
  id_  = dsrc->id_;
}
...
void
VideoBufEvent::serialize (Archive& ar, const std::uint32_t /* file_version */)
{
