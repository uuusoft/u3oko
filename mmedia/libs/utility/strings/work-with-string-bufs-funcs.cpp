  ...
  std::string&                 buf) -> bool
{
  bool ret = false;
  std::for_each (
    vals, vals + vals_count, [&buf, &ret] (const syn::replace_val_type& val) -> void {
      ...
      while ((replace_str = strstr (buf.c_str (), val.first.c_str ())))
      {
        ...
        memset (::libs::utility::casts::reinterpret_cast_helper< void* > (const_cast< char* > (replace_str)), ' ', val.first.length ());
  ...
  std::string&                 dstbuf) -> bool
{
  ...
  std::for_each (
    vals,
    vals + vals_count,
    [&dstbuf, &tempbuf, &ret] (const syn::replace_val_type& val) -> void {
