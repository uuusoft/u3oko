...
auto
create_folder (const std::string& path) -> bool
{
  if (path.empty ())
  {
...
auto
is_folder (const std::string& ppath) -> bool
{
...
auto
get_name_from_path (const std::string& path) -> std::string
{
...
auto
prepare_for_file_name (const std::string_view& src) -> std::string
{
