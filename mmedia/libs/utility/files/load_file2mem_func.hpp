...
bool load_file2mem (const std::string& file_path, ::libs::utility::mem::IBlockMem::ptr& ret);

/// Свободная функция сохранения блока памяти в файл
/// \param[in]  file_path  путь к файлу
/// \param[out] ret        блок памяти с новым содержимым файла
/// \return     true, если загрузка прошла успешно
bool save_mem2file (const std::string& file_path, const ::libs::utility::mem::IBlockMem::ptr& ret);
...
load_file2mem (const std::string& file_path, TContainer& ret)
{
  ...
  ret.resize (size);
  file.read (::libs::utility::casts::reinterpret_cast_helper< char* > (ret.data ()), size);
  // ret[size] = '\0';
  return true;
}
