/**
\file       expanded-times-helpers.cpp
\author     Erashov Anton erashov2026@proton.me
\date       01.01.2017
\project    u3_helpers_lib
*/
#include "../../utility-lib-includes_int.hpp"
#include "mmedia/libs/ilink/consts/libs-ilink-const-vals.hpp"
#include "mmedia/libs/events/includes.hpp"
#include "mmedia/libs/link/libs-link-includes.hpp"
#include "expanded-times-helpers.hpp"

namespace libs::utility::statistic::helpers::functors
{
AddOpTime::AddOpTime (
  ExpandedTimes&                  obj,
  ExpandedTimes::key_storage_type id_set,
  ExpandedTimes::key_storage_type id_element) :
  obj_ (obj),
  skip_ (false),
  id_set_ (std::move (id_set)),
  id_element_ (std::move (id_element))
{
  obj_.begin (id_set_, id_element_);
}


AddOpTime::~AddOpTime ()
{
  if (skip_)
  {
    obj_.end_skip ();
  }
  else
  {
    obj_.end ();
  }
}


void
AddOpTime::set_skip ()
{
  skip_ = true;
}


#ifdef U3_DISABLE_AS_0_FOR_CLANG_TIDY
void
change_last_name (const std::string& name)
{
  obj_.change_last_name (name);
}
#endif

void
dump (ExpandedTimes& obj, ::libs::link::ILink::ptr& logger)
{
#ifdef U3_DISABLE_AS_0_FOR_CLANG_TIDY
  ExpandedTimes::infos_type infos;
  obj.get (infos);
  for (const std::string& info : infos)
  {
    U ~3_MSG2LOGGER(logger, info, "statxx", ::libs::events_base::props::modules::log::LogLevels::info);
  }
#endif
}
}   // namespace libs::utility::statistic::helpers::functors
