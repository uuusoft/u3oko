...
auto
ExpandedTimes::find_time (const key_storage_type& id_set) -> ExpandedTimes::storages_type::iterator
{
  ...
  const key_storage_type& id_element)
{
  ...
  ops_.emplace_back (id_set, id_element);
  times_.push_back (ticks);
}


auto
ExpandedTimes::end () -> bool
{
  time_type ticks = U3_GET_CURRENT_TIME;
  ...
  find_element->second.min_ = ::libs::utility::statistic::count_time_type (0) == find_element->second.min_ ? diff : std::min< duration_type > (diff, find_element->second.min_);
