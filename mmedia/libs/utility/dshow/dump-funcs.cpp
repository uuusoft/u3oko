...
dump (const AM_MEDIA_TYPE* minfo)
{
  ...
  U3_XLOG_WARN (::libs::utility::utils::to_string (utils::cuuid (minfo->formattype)) + " majortype=" + ::libs::utility::utils::to_string (utils::cuuid (minfo->majortype)) + " subtype=" + ::libs::utility::utils::to_string (utils::cuuid (minfo->subtype)));
  ...
  if (FORMAT_VideoInfo == minfo->formattype)
  {
    ...
    auto* info = ::libs::utility::casts::reinterpret_cast_helper< VIDEOINFOHEADER* > (minfo->pbFormat);
    dump (*info);
    return;
  }
  ...
  if (FORMAT_VideoInfo2 == minfo->formattype)
  {
    ...
    auto* info = ::libs::utility::casts::reinterpret_cast_helper< VIDEOINFOHEADER2* > (minfo->pbFormat);
    dump (*info);
    return;
  }
...
dump (const IPinPtr& ipin)
{
  ...
  HRESULT                    hr = ipin->QueryInterface (IID_IAMStreamConfig, ::libs::utility::casts::reinterpret_cast_helper< void** > (&iam_stream.update ()));
