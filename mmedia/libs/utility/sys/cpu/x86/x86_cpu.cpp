...
auto
fxgetbv (uint32_t index) -> uint64_t
{
  std::uint32_t eax = 0;
  std::uint32_t edx = 0;
...
#  if defined(U3_COMPILER_MSC)
    __cpuid (::libs::utility::casts::reinterpret_cast_helper< std::int32_t* > (regs), regs_eax[indx]);
#  else
    __get_cpuid (regs_eax[indx], &regs[0], &regs[1], &regs[2], &regs[3]);
#  endif

    *::libs::utility::casts::reinterpret_cast_helper< std::uint32_t* > (buf + 16 * indx + 0)  = regs[0];
    *::libs::utility::casts::reinterpret_cast_helper< std::uint32_t* > (buf + 16 * indx + 4)  = regs[1];
    *::libs::utility::casts::reinterpret_cast_helper< std::uint32_t* > (buf + 16 * indx + 8)  = regs[2];
    *::libs::utility::casts::reinterpret_cast_helper< std::uint32_t* > (buf + 16 * indx + 12) = regs[3];
  }
...
auto
get_exts_cpu (SupportExtensionCPU& ret) -> bool
{
...
#  if defined(U3_COMPILER_MSC)
  __cpuid (::libs::utility::casts::reinterpret_cast_helper< std::int32_t* > (regs), 0);
#  else
...
#  if defined(U3_COMPILER_MSC)
    __cpuid (::libs::utility::casts::reinterpret_cast_helper< std::int32_t* > (regs), check_eax);
#  else
...
#  endif
    ...
    {
#  if defined(U3_COMPILER_MSC)
      __cpuid (::libs::utility::casts::reinterpret_cast_helper< std::int32_t* > (regs), 1);
#  else
