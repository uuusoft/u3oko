#pragma once
/**
\file       size-changed-event.hpp
\date       01.01.2017
\project    u3_events_gui
*/

namespace libs::events_gui::events
{
class SizeChangedEvent : public BaseGUIEvent
{
  friend class boost::serialization::access;

  protected:
  struct Acessor {
    explicit Acessor (int) {};
  };

  public:
  //  ext types
  U3_ADD_POINTERS_TO_SELF (SizeChangedEvent)
  U3_ADD_MAKE_SHARED_THIS (SizeChangedEvent)
  U3_ADD_DELETE_MOVE_COPY (SizeChangedEvent)

  explicit SizeChangedEvent (
    const Acessor&                             = Acessor (0),
    ::libs::events_gui::utils::ISize::ptr size = ::libs::events_gui::utils::ISize::ptr ());

  virtual ~SizeChangedEvent ();

  virtual TypeEvents get_type () const override;

  ::libs::events_gui::utils::ISize::ptr get_size () const;
  void                                  set_size (const ::libs::events_gui::utils::ISize::ptr&);

  static constexpr auto
  gen_get_mid () -> const IEvent::hid_type&
  {
    static constexpr const char*            chret = "libs/events_gui/events/props/size-changed-event";
    static constexpr const IEvent::hid_type ret { chret };
    return ret;
  }

  private:
  U3_ADD_SUPER_CLASS (::libs::events_gui::events::BaseGUIEvent)

  ::libs::events_gui::utils::ISize::ptr size_;   //<

  friend class boost::serialization::access;

  template< class Archive >
  void serialize (Archive& arh, const std::uint32_t /* file_version */);

  // IEvent overrides
  virtual auto get_mid_int () const -> const ::libs::events::IEvent::hid_type& override;
  virtual auto clone_int (const ::libs::events::Deeps&) const -> ::libs::events::IEvent::ptr override;
  virtual auto copy_int (const IEvent::craw_ptr) -> void override;
};
}   // namespace libs::events_gui::events

BOOST_CLASS_EXPORT_KEY (::libs::events_gui::events::SizeChangedEvent);
