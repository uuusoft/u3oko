#pragma once
/**
\file       base-gui-event.hpp
\author     Erashov Anton erashov2026@proton.me
\date       01.01.2017
\project    u3_events_gui
*/

namespace libs::events_gui::events
{
/// Базовый класс всех событий подсистемы GUI
class BaseGUIEvent : public ::libs::events_base::Event
{
  friend class boost::serialization::access;

  protected:
  struct Acessor {
    explicit Acessor (int) {};
  };

  public:
  //  ext types
  U3_ADD_POINTERS_TO_SELF (BaseGUIEvent)
  U3_ADD_MAKE_SHARED_THIS (BaseGUIEvent)
  U3_ADD_DELETE_MOVE_COPY (BaseGUIEvent)

  BaseGUIEvent (const Acessor& = Acessor (0));
  virtual ~BaseGUIEvent ();

  virtual TypeEvents get_type () const;

  static constexpr auto
  gen_get_mid () -> const IEvent::hid_type&
  {
    static constexpr const char* chret = "libs/events_gui/events/props/base-gui-event";
    static constexpr const IEvent::hid_type ret { chret };
    return ret;
  }

  protected:
  virtual auto copy_int (const IEvent::craw_ptr) -> void override;

  private:
  U3_ADD_SUPER_CLASS (::libs::events_base::Event)

  friend class boost::serialization::access;

  template< class Archive >
  void serialize (Archive& arh, const std::uint32_t /* file_version */);

  // IEvent overrides
  virtual auto get_mid_int () const -> const ::libs::events::IEvent::hid_type& override;
  virtual auto clone_int (const ::libs::events::Deeps&) const -> ::libs::events::IEvent::ptr override;
};
}   // namespace libs::events_gui::events

BOOST_CLASS_EXPORT_KEY (::libs::events_gui::events::BaseGUIEvent);
