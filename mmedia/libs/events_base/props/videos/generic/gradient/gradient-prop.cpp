...
{
  channels_.emplace_back (::utils::dbufs::video::consts::offs::lit, ChannelGradient ());
  channels_.emplace_back (::utils::dbufs::video::consts::offs::sat, ChannelGradient ());
  channels_.emplace_back (::utils::dbufs::video::consts::offs::hue, ChannelGradient ());
}


auto
GradientProp::get_mid_int () const -> const ::libs::events::IEvent::hid_type&
{
  return GradientProp::gen_get_mid ();
}


auto
GradientProp::clone_int (const ::libs::events::Deeps& deep) const -> ::libs::events::IEvent::ptr
{
...
auto
text2grad_type (const std::string& text) -> Gradients
{
    ...
    if (std::string ("range") == name_param.as_string ())
    {
      ...
      if (vals.size () >= 2)
      {
        ...
        if (vals_after.size () >= 2)
        {
          ...
          const auto        hue = boost::lexical_cast< std::int16_t, std::string > (vals_after[1]);
