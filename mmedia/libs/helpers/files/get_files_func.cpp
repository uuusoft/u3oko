...
{
  const std::string         path = ::libs::helpers::files::make_path (root_path, add_path);
  ...
  while (it_dir != boost::filesystem::directory_iterator ())
  {
    ...
    const auto  add_name  = ::libs::helpers::files::make_path (add_path, file_name);
    ...
    if (is_directory (full_path))
    {
      ...
      {
        enums.folders_.push_back (NodeEnumFiles ());
