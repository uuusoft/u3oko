...
dump (const AM_MEDIA_TYPE* minfo)
{
  ...
  if (FORMAT_VideoInfo == minfo->formattype)
  {
    ...
    VIDEOINFOHEADER* info = U3_CAST_REINTERPRET< VIDEOINFOHEADER* > (minfo->pbFormat);
    dump (*info);
    return;
  }
  ...
  if (FORMAT_VideoInfo2 == minfo->formattype)
  {
    ...
    VIDEOINFOHEADER2* info = U3_CAST_REINTERPRET< VIDEOINFOHEADER2* > (minfo->pbFormat);
    dump (*info);
    return;
  }
...
dump (const IPinPtr& ipin)
{
  ...
  HRESULT                    hr = ipin->QueryInterface (IID_IAMStreamConfig, U3_CAST_2VOID (&iam_stream.update ()));
