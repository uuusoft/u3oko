...
#  if defined(U3_COMPILER_MSC)
    __cpuid (U3_CAST_REINTERPRET< std::int32_t* > (regs), regs_eax[indx]);
#  else
    __get_cpuid (regs_eax[indx], &regs[0], &regs[1], &regs[2], &regs[3]);
#  endif

    *U3_CAST_REINTERPRET< std::uint32_t* > (buf + 16 * indx + 0)  = regs[0];
    *U3_CAST_REINTERPRET< std::uint32_t* > (buf + 16 * indx + 4)  = regs[1];
    *U3_CAST_REINTERPRET< std::uint32_t* > (buf + 16 * indx + 8)  = regs[2];
    *U3_CAST_REINTERPRET< std::uint32_t* > (buf + 16 * indx + 12) = regs[3];
  }
...
#  if defined(U3_COMPILER_MSC)
  __cpuid (U3_CAST_REINTERPRET< std::int32_t* > (regs), 0);
#  else
...
#  if defined(U3_COMPILER_MSC)
    __cpuid (U3_CAST_REINTERPRET< std::int32_t* > (regs), check_eax);
#  else
...
#  endif
    ...
    {
#  if defined(U3_COMPILER_MSC)
      __cpuid (U3_CAST_REINTERPRET< std::int32_t* > (regs), 1);
#  else
