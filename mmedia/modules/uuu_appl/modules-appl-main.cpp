...
signal_stop_process (int signal)
{
  ...
  U3_XLOG_DEV ("signal_stop_process" + VTOLOG (signal) + VTOLOG (stop_counter));
  if (::modules::uuu_appl::g_module_appl && ::modules::uuu_appl::g_module_appl->impl () && 0 == stop_counter)
  {
    U3_XLOG_DEV ("stop process begin" + VTOLOG (signal));
    ::modules::uuu_appl::g_module_appl->impl ()->appl_force_stop ();   // Без затей и блокировок, просто выставляем флаг.
  }
  if (stop_counter > 32)
  {
    std::abort ();   // debug
...
prepare_process ()
{
  U3_XLOG_DEV ("START prepare_process");
  ::libs::ilink::appl::base::register_events_for_module ();
  auto istorage = ::libs::iproperties::helpers::get_storage ();

  istorage->set_prop (
    ::libs::properties::consts::keys::demons_property,
    std::make_shared< ::libs::iproperties::vers::demon::IDemonsProperty > ());
...
#  endif
  ...
  U3_XLOG_DEV ("update ::libs::properties::consts::keys::shared_os_property");
  istorage->set_prop (
    ::libs::properties::consts::keys::shared_os_property,
    std::make_shared< ::libs::iproperties::vers::system::ISystemProperty > ());

  // debug
  // U3_XLOG_DEV ("check ::libs::properties::consts::keys::shared_os_property");
  // auto test = istorage->get_prop (::libs::properties::consts::keys::shared_os_property);

  istorage->set_prop (
    ::libs::properties::consts::keys::specific_os_property,
    std::make_shared< ::libs::iproperties::vers::system::SystemWin32Property > ());

#elif defined(U3_OS_ANDROID)
#else
#  error select OS
#endif
  U3_XLOG_DEV ("STOP prepare_process");
}
...
init_process (int argc, char* argv[])
{
  ...
  U3_XLOG_DEV ("init_process1" + VTOLOG (argc) + TOLOG (curfolder));
  ::modules::uuu_appl::up_links (curfolder, argc, argv);
  U3_XLOG_DEV ("init_process2: up links" + VTOLOG (argc) + TOLOG (curfolder));
  U3_CHECK (::modules::uuu_appl::check_process (), "call check_process");
  U3_XLOG_DEV ("init_process2" + TOLOG (curfolder) + TOLOG (::modules::uuu_appl::g_init_appl.lib_name_))
  ::modules::uuu_appl::g_module_appl = std::make_shared< ::libs::link::appl::IApplicationProxy > (curfolder, ::modules::uuu_appl::g_init_appl.lib_name_);
  ::modules::uuu_appl::g_module_appl->impl ()->appl_init (::modules::uuu_appl::g_init_appl);
}
...
work_process ()
{
  U3_XLOG_DEV ("START work_process");
  ::modules::uuu_appl::g_module_appl->impl ()->appl_work ();
}
...
{
  U3_XLOG_DEV ("START deinit_impl_application");
  auto applimpl = ::modules::uuu_appl::g_module_appl->impl ();
  U3_ASSERT (applimpl);
...
deinit_properties ()
{
  U3_XLOG_DEV ("START deinit_properties");
  ...
  U3_XLOG_DEV ("STOP deinit_properties");
}
...
deinit_links ()
{
  U3_XLOG_DEV ("START deinit_links");
...
deinit_process ()
{
  U3_XLOG_DEV ("START deinit_process");
  deinit_impl_application ();
  deinit_properties ();
  deinit_links ();
}
...
setup_signals_catch ()
{
  U3_XLOG_DEV ("START setup_signals_catch");
