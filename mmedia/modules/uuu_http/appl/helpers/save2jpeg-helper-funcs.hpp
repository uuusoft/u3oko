...
{
  U3_ASSERT (buf);
  auto head = U3_CAST_REINTERPRET< const ::dlls::codecs::codec_gen::HeaderIFrame* > (buf);
  ...
  syn::ZipDataEvent::raw_ptr image)
{
  ...
  auto       head        = U3_CAST_REINTERPRET< const ::dlls::codecs::codec_gen::HeaderIFrame* > (&image->get_zip ()[0]);
  ...
  try
  {
    ...
    file.write (U3_CAST_REINTERPRET< const char* > (cdata), src_size_res);
    file.flush ();
  }
