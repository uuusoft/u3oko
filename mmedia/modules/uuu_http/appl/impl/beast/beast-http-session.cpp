...
{
  U3_XLOG_DEV ("http_session::http_session::on_create" + VTOLOG (U3_CAST_PTR2INT (this)));
...
http_session::do_read ()
{
  ...
  // Set the timeout.
  stream_.expires_after (std::chrono::seconds (modules::uuu_http::appl::consts::expires_after));
...
http_session::on_read (boost::beast::error_code ec, std::size_t bytes_transferred)
{
  ...
  {
    U3_XLOG_DEV ("http_session::on_read UPGRADE to websocket" + VTOLOG (U3_CAST_PTR2INT (this)));
...
{
  U3_XLOG_DEV ("http_session::http_session::do_close" + VTOLOG (U3_CAST_PTR2INT (this)));
  // Send a TCP shutdown
  boost::beast::error_code ec;
