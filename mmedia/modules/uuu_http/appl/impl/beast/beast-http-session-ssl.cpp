...
{
  U3_XLOG_DEV ("http_session_ssl::http_session_ssl::on_create" + VTOLOG (U3_CAST_PTR2INT (this)));
  static_assert (modules::uuu_http::appl::consts::queue_limit > 0, "queue limit must be positive");
}
...
{
  // Set the timeout.
  boost::beast::get_lowest_layer (stream_).expires_after (std::chrono::seconds (modules::uuu_http::appl::consts::expires_after));
...
http_session_ssl::do_read ()
{
  ...
  parser_->body_limit (modules::uuu_http::appl::consts::body_limit);
...
http_session_ssl::on_read (boost::beast::error_code ec, std::size_t bytes_transferred)
{
  ...
  {
    U3_XLOG_DEV ("http_session_ssl::on_read UPGRADE to websocket" + VTOLOG (U3_CAST_PTR2INT (this)));
  ...
  if (response_queue_.size () < modules::uuu_http::appl::consts::queue_limit)
  {
    do_read ();
  }
#endif
...
{
  U3_XLOG_DEV ("http_session_ssl::http_session_ssl::do_close" + VTOLOG (U3_CAST_PTR2INT (this)));
