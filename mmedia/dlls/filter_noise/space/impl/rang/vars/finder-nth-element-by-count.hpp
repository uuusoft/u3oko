...
class FinderNthElementByCount final
{
  ...
  update_min_max (
    ...
    std::uint32_t            sstride)
  {
#if defined(SKIP_OPERATIONS_FOR_FINDER_NELEMENT)
    //  debug
    min_ = rand () % 20;
    max_ = min_ + rand () % 100;
    return;
    ...
    for (int indxy = 0; indxy < size_core; ++indxy)
    {
      ...
      U3_FAST_MOVE_CPTR (minmaxbeg, sstride);
    }
  }
    ...
    std::uint32_t            sstride)
  {
    ...
    //  уменьшаем частоты удаленного левого столбца.
    {
      ...
      for (int indxy = 0; indxy < size_core; ++indxy)
      {
        ...
        --update_counter (val);
        U3_FAST_MOVE_CPTR (del_val, sstride);
      }
    }
...

    {
      ...
      for (int indxy = 0; indxy < size_core; ++indxy)
      {
        ...
        ++update_counter (val);
        U3_FAST_MOVE_CPTR (add_val, sstride);
      }
    }
    ...
    std::uint32_t            sstride)
  {
    ...
    for (int indxy = 0; indxy < size_core; ++indxy)
    {
...
#if defined(DEBUG_ALG_SORT_FUND_RANG_ELEMENT)
      ...
      U3_FAST_MOVE_CPTR (cur_val, sstride);
    }
  }
  ...
  source_value_type   min_;              //< Минимальный индекс счетчика, с значением отличным от 0
  source_value_type   max_;              //< Максимальный индекс счетчика, с значением отличным от 0
  std::int32_t        last_size_core_;   //< Последний использованный размер ядра фильтрации. Может меняться по условию
};
}   // namespace dlls::filter_noise::space::impl::rang::vars
