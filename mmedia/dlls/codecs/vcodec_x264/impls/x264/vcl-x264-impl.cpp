...
#  include "vcl-x264-impl.hpp"

namespace dlls::codecs::vcodec_x264::impls::x264
{
VclX264Impl::VclX264Impl ()
// coder_ (nullptr),
// decoder_ (nullptr)
{
}
...
void
VclX264Impl::create_coder_int (
  const syn::VideoCodecProp& cinfo,
  const syn::AllocBufInfo&  coder_frame_info)
{
#  if 0
  const auto birate = ::libs::ievents::props::videos::generic::codec::to_bitrate_x264 (cinfo.plane_, coder_frame_info.width_, coder_frame_info.height_);
  U3_LOG_DATA_DEV ("create encoder" + VTOLOG (birate) + VTOLOG (coder_frame_info.width_) + VTOLOG (coder_frame_info.height_));
...
#    if 1
  ...
  ext_params.iPicWidth      = coder_frame_info.width_;
  ext_params.iPicHeight     = coder_frame_info.height_;
...
void
VclX264Impl::create_decoder_int ()
{
#  if 0
  U3_ASSERT (!decoder_);
...
void
VclX264Impl::destroy_decoder_int ()
{
#  if 0
  if (!decoder_)
  {
...
bool
VclX264Impl::compress_frame_int (const syn::VideoCodecProp& cinfo, EncodeFrameState& state)
{
#  if 0
  out_size = 0;
  dst.check ("dst x264Impl::compress_frame");
  U3_CHECK (coder_, "empty coder");

  std::uint8_t*                           dbuf       = dst.ubuf ();
  syn::HeaderIFrame* head        = U3_CAST_CODECS< syn::HeaderIFrame* > (dbuf);
  int                                      from_header = 0;
  ::libs::optim::io::ProxyBuf             lsrc (src_buf.get (), "src_buf x264Impl::compress_frame");

  out_size = sizeof (syn::HeaderIFrame);
  head->reset ();

  // const std::int32_t frame_size = lsrc.width_ * lsrc.height_ * 3 / 2;
  SFrameBSInfo   frame_info;
  SSourcePicture source_pic;

  // buf.SetLength (frame_size);
  ...
  //source_pic.iColorFormat = videoFormatRGB;
  ...
  ::utils::dbufs::video::IVideoBuf::fragments_type fragments;
  int                                                size_fragments = 0;
  ...
  for (int indx = 0; indx < frame_info.iLayerNum; ++indx)
  {
    ...
    for (int nindx = 0; nindx < linfo.iNalCount; ++nindx)
    {
      ...
      U3_ASSERT (*(reinterpret_cast< std::uint32_t* > (naps_buf)) == 0x01000000);
      //*U3_CAST_REINTERPRET<int*> (naps_buf) = size_nal - sizeof (int);
      ::libs::helpers::mem::u3copy (naps_buf, dbuf + out_size + size_fragments + size_fragment, size_nal);
  ...
  head->cinfo_.nocolor_ = colored ? false : true;   //�������������� �� �����, �.�. � ������������ ����� ���� ����������� ������ � ������ ��� ��� ����������� ���������� � ��������.

  std::copy (????::libs::helpers::uids::codecs::x264.get_vals ().begin (), ::libs::helpers::uids::codecs::x264.get_vals ().end (), head->base_part_.guid_);
...
#  if 0
  ...
  try
  {
    if (!decoder_)
    {
      ...
      U3_CHECK (0 == ires, "call decoder_->Initialize, " << ires);
...
#    endif
    // for Decoding only
    U3_LOG_DATA_DEV (VTOLOG (src_size_res));
    ...
    idst->buf_alloc (
      syn::AllocBufInfo (
        ...
        ::utils::dbufs::BufFlags::convolution_support));
...
VclX264Impl::get_support_frame_format_int (const syn::CodecModes& codec_mode) const
{
