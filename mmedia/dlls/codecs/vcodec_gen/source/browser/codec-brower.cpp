...
{
auto
CodecBrower::get_codec (const libs::utility::uids::minor::id_val& id) -> std::string
{
  lock_type lock (mtx_);
  refresh_list_codecs ();
...
CodecBrower::refresh_list_codecs ()
{
  ...
  for (const auto& file : files.files_)
  {
    ...
    libs::utility::uids::minor::id_val formats[4];
    impl.get_codec_info_ (formats);

    for (const auto& format : formats)
    {
      if (format == libs::utility::uids::minor::id_val::unknown)
      {
        continue;
      }
