...
class CodecBrowser final
{
  ...
  /// \return     имя файла
  auto get_codec (const syn::minor::id_val&) -> std::string;

  private:
  //  internal typess
  using sync_type        = std::mutex;
  using lock_type        = std::scoped_lock< sync_type >;
  using format2file_type = std::unordered_multimap< syn::minor::id_val, std::string >;
