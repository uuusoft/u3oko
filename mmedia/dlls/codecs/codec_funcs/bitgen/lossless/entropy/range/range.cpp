...
/* per outputbyte.                                             */
void
start_encoding (rangecoder *rc, char c, int initlength)
{
  ...
  while (RNGC.range <= Bottom_value) /* do we need renormalisation?  */
  {
    if (RNGC.low < (code_value) 0xff << SHIFT_BITS) /* no carry possible --> output */
    {
      ...
      for (; RNGC.help; RNGC.help--)
        M_outbyte (0xff);
      RNGC.buf = (std::uint8_t) (RNGC.low >> SHIFT_BITS);
    }
    else if (RNGC.low & Top_value) /* carry now, no future carry */
    {
      M_outbyte (RNGC.buf + 1);
      for (; RNGC.help; RNGC.help--)
        M_outbyte (0);
      RNGC.buf = (std::uint8_t) (RNGC.low >> SHIFT_BITS);
    }
...
void
encode_freq (rangecoder *rc, freq sy_f, freq lt_f, freq tot_f)
{
  code_value r, tmp;
  enc_normalize (rc);
  r   = RNGC.range / tot_f;
...
void
encode_shift (rangecoder *rc, freq sy_f, freq lt_f, freq shift)
{
  code_value r, tmp;
  enc_normalize (rc);
  r   = RNGC.range >> shift;
...
int
start_decoding (rangecoder *rc)
{
  int c = M_inbyte;
  if (c == EOF)
    return EOF;
  RNGC.buf   = (std::uint8_t) M_inbyte;
  RNGC.low   = RNGC.buf >> (8 - EXTRA_BITS);
...
static Inline void
dec_normalize (rangecoder *rc)
{
  while (RNGC.range <= Bottom_value)
  {
    RNGC.low = (RNGC.low << 8) | ((RNGC.buf << EXTRA_BITS) & 0xff);
    RNGC.buf = (std::uint8_t) M_inbyte;
    RNGC.low |= RNGC.buf >> (8 - EXTRA_BITS);
    RNGC.range <<= 8;
  }
...
freq
decode_culfreq (rangecoder *rc, freq tot_f)
{
  freq tmp;
  dec_normalize (rc);
...
freq
decode_culshift (rangecoder *rc, freq shift)
{
  freq tmp;
  dec_normalize (rc);
...
/* tot_f is the total interval length ( total frequency sum)  */
void
decode_update (rangecoder *rc, freq sy_f, freq lt_f, freq tot_f)
{
  code_value tmp;
  tmp = RNGC.help * lt_f;
  RNGC.low -= tmp;
...
/* rc is the range coder to be used                          */
std::uint8_t
decode_byte (rangecoder *rc)
{
  std::uint8_t tmp = decode_culshift (rc, 8);
  decode_update (rc, 1, tmp, (freq) 1 << 8);
  return tmp;
}

std::uint16_t
decode_short (rangecoder *rc)
{
  std::uint16_t tmp = decode_culshift (rc, 16);
