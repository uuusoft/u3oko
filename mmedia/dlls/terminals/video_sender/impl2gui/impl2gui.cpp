...
{
  ::libs::link::ILink::ptr helper = ::libs::iproperties::helpers::get_prop_links ()->get_links_lockfree ()[syn::mids::mdata2appl].lock ();
  if (!helper)
  {
...
Impl2Gui::fill_frame (const syn::TransformInfo& info, void* pmem) -> bool
{
  ...
  try
  {
    ...
    std::uint32_t off_buf = header->off_bufs_;
    ...
    for (std::size_t bindx = 0; bindx < std::size (buf_indxs); ++bindx)
    {
      ...
      *buf_offs[bindx]    = off_buf;
      ...
      ::libs::utility::mem::mem_copy_raw (utils::dbufs::video::helpers::get_const_data (buf), dest_buf, (*buf)[::utils::dbufs::MemVars::size_data]);
  ...
  const ::libs::link::ILink::ptr& helper)
{
  ...
  else
  {
    ...
    if (fast_buf2gui_.use_count () <= 1)
    {
      ...
      auto [evnt, revnt] = ::libs::iproperties::helpers::create_event< ::libs::events_gui::events::VideoBufEvent > ();
      revnt->set_buf (fast_buf2gui_);
      // revnt->set_id (finfo.rprops_->get_id ());
