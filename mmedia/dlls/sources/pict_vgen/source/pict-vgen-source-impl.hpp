...
class SourceImpl final : public ::dlls::sources::gen_lib::ISourceImpl
{
  ...
  using sources_type          = std::vector< ::libs::events_media::events::DataSourceInfo >;       //<
  using val_bufs_storage_type = std::pair< std::int32_t, LoadedImage >;                            //<
  using bufs_storage_type     = boost::unordered_flat_map< std::string, val_bufs_storage_type >;   //<
  ...
  virtual auto get_sources_int (SourceImpl::sources_type&) -> void override;
  virtual auto get_raw_data_int (syn::pkeys2bufs_type&, syn::tevents_type*) -> void override;
  virtual auto set_cpu_int (syn::CpuExts) -> void override;
  virtual auto update_source_info_int (const syn::SourceImplInfo&) -> void override;

  auto stop_and_free () -> void;
  auto refresh_files () -> void;
  auto print_images2buf (utils::dbufs::video::IVideoBuf::ptr&) -> void;
  auto sync_internal_structures () -> void;
  auto load_image2buf (const std::string&) -> LoadedImage;
  auto is_image_valid (const std::string&) -> bool;

  std::string                name_file_;                                                   //<
  syn::NodeEnumFiles         enum_files_;                                                  //<
