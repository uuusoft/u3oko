  ...
  const std::function< void (std::uint32_t indx, TPx& val) >& func)
{
  ...
  if (!begbuf || 0 == size || buf->get_flag (BufFlags::null))
  {
    return;
  }
  ...
  std::uint32_t       off_dest = 0)
{
  ...
  ::libs::helpers::mem::u3copy (rbuf, buf->get_buf () + off_dest, size);
}


inline void
copy_video_buf2vector (
  ...
  std::vector< std::uint8_t >&              out)
{
  ...
  ::libs::helpers::mem::u3copy (raw_buf, &out[0], size_buf);
}
}   // namespace utils::dbufs::video::helpers
