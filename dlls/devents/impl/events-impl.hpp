...
class EventsImpl : public ::libs::events::io::IEvents
{
  ...
  using str2gen_events_type     = std::unordered_map<hid_type, create_event_funct_type>;
  using cast_event_funct_type   = std::function<IEvent::craw_ptr (IEvent::craw_ptr)>;
  using str2cast_events_type    = std::unordered_map<hid_type, cast_event_funct_type>;
  ...
  virtual ~EventsImpl ();
  //  IEvents overrides
  virtual IEvent::ptr get (const hid_type& _id) override;
  virtual IEvent::ptr clone (const IEvent::craw_ptr _src, const ::libs::events::DeepEventCloneType& _type) override;
  virtual void*       dcast (IEvent::craw_ptr _src, const hid_type& _id) override;
