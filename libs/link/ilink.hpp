...
/**
  \brief  Интерфейс, который должна поддерживать реализация связи между модулями системы.
  */
class ILink
{
  public:
  ...
  /**
    ...
    \return   true, при успехе.
    */
  bool destroy (const TypeDestroyLink& _type = TypeDestroyLink::soft);
  /**
  ...
  IEvent::ptr send_msg (
    IEvent::ptr                     _msg,
    const details::TypeSyncCall&    _sync = details::TypeSyncCall::async,
    const details::TypeRequestCall& _req  = details::TypeRequestCall::set,
  ...
  virtual bool               destroy_int (const TypeDestroyLink& _type)                                                                                               = 0;
  ...
  virtual IEvent::ptr        send_msg_int (IEvent::ptr _msg, const details::TypeSyncCall& _sync, const details::TypeRequestCall& _req, const ISeqEvent::id_type& _id) = 0;
};
