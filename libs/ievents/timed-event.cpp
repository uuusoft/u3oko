//This is an independent project of an individual developer. Dear PVS-Studio, please check it.
//PVS-Studio Static Code Analyzer for C, C++ and C#: http://www.viva64.com
/**
\file       timed-event.cpp
\author     Anton Erashov /eai/ skype: ytkoduff; emails: erashov@uuusoft.com, erashov2004@yandex.ru
\date       01.01.2017
\copyright  www.uuusoft.com
\project    uuu_ievents
\brief    
*/
#include "mmedia/defines/defines.hpp"
#include "mmedia/defines/includes.hpp"
#include "includes_int.hpp"
#include "timed-event.hpp"

namespace libs { namespace ievents {

TimedEvent::TimedEvent (const Acessor& _ph) :
  time_ (boost::posix_time::microsec_clock::universal_time ())
{
  property_name_ = gen_get_type_text_id ();
}


TimedEvent::~TimedEvent ()
{}


::libs::events::IEvent::ptr
TimedEvent::clone_int (const ::libs::events::TypeCloneEvent& _deep) const
{
  return helper_impl_clone_funct<TimedEvent> (this, _deep);
}


boost::posix_time::ptime
TimedEvent::get_time () const
{
  return time_;
}


void
TimedEvent::copy_int (const IEvent::craw_ptr _src)
{
  CHECK_STATE_COPY_EVENT (TimedEvent);
  super::copy_int (_src);
  time_ = _dsrc->time_;
  return;
}


template <class Archive>
void
TimedEvent::serialize (Archive& ar, const unsigned int /* file_version */)
{
  ar& BOOST_SERIALIZATION_BASE_OBJECT_NVP (super);
  ar& BOOST_SERIALIZATION_NVP (time_);
  return;
}

}}      // namespace libs::ievents

BOOST_CLASS_EXPORT_IMPLEMENT (::libs::ievents::TimedEvent);
SERIALIZE_TYPE_TO_ARCHIVES (::libs::ievents::TimedEvent);
